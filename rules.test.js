...
t('card values', () => {
  eq(E.cardValue('XR0'), 50); eq(E.cardValue('2S0'), 20); eq(E.cardValue('AH0'), 20);
  eq(E.cardValue('KD0'), 10); eq(E.cardValue('8C0'), 10); eq(E.cardValue('7S0'), 5);
  eq(E.cardValue('4H0'), 5); eq(E.cardValue('3S0'), 5);
});
t('3 players deals 11 and 11, five decks', () => {
  const s = G.createGame(['A', 'B', 'C']);
  G.startRound(s, mulberry(1));
...
t('books, melds, leftovers and red threes all count', () => {
  ...
  const rows = G.scoreRound(s);
  const r = rows[0];
  eq(r.books, 800); eq(r.meldPts, 190); eq(r.left, 10);
  eq(r.threes, -100); eq(r.out, 100);
  eq(r.total, 800 + 190 - 100 + 100 - 10);
});
t('cards left in an unplayed foot still count against you', () => {
  const s = G.createGame(['A', 'B']);
  G.startRound(s, mulberry(2));
